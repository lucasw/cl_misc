...
__kernel void hello(__read_only image2d_t image,
  ...
  for (int i = 0; i < num; i++) {
    ...
    for (int j = 0; j < num; j++) {
      int aa = sig;
      const int2 pos = {get_global_id(0) + (j - num / 2) * aa,
                        get_global_id(1) + (i - num / 2) * aa};
  ...
  /*
    ...
    */
  // write_imageui(im_out, pos, get_local_id(0)*fr + get_local_id(1)*fr); //
  // (val + val2)/2 );
};
